...
class SELDMetrics(object):
    ...
    def __init__(self, doa_threshold=20, req_onscreen=True,
    ...
    def compute_seld_scores(self):
        ...
        :return: returns both location-sensitive detection scores and class-sensitive localization scores:
            F score, angular error, onscreen accuracy, and classwise results
        """
        ...
        if self._average == 'micro':
            ...
            classwise_results = np.array([F, AngE, OnscreenAq])
        ...
        return F, AngE, OnscreenAq, classwise_results
...
class ComputeSELDResults(object):
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        F, AngE, OnscreenAq, classwise_results = eval.compute_seld_scores()

        if is_jackknife:
            global_values = [F, AngE, OnscreenAq]
            ...
            for leave_file in pred_files:
                ...
                F, AngE, OnscreenAq, classwise_results = eval.compute_seld_scores()
                leave_one_out_est = [F, AngE, OnscreenAq, classwise_results]
                if len(classwise_results):
            ...
            return ([F, conf_interval[0]], [AngE, conf_interval[1]], [OnscreenAq, conf_interval[2]],
                    [classwise_results, np.array(conf_interval)[3:].reshape(5, 13, 2) if len(classwise_results) else []])
