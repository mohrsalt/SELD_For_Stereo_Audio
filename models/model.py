...
class SED_DOA(nn.Module):
    def __init__(self, in_channel, in_dim):
        ...
        num_layers = 8 ## 8,10,12-> vanilla nercslip with 128 melbins, then we decide on 256 based on the trend, then we try 8-2 for with onepeace and decide melbins based on prev trends
...
###DEVIN
class SED_DOA_OPENPEACE(nn.Module):
    def __init__(self, in_channel, in_dim):
        super().__init__()
        self.resnet = resnet18_nopool(in_channel=in_channel)
        embedding_dim = in_dim // 32 * 256
    ...
    ##Wav
class SED_DOA_wav(nn.Module):
    def __init__(self, in_dim):
        super().__init__()
        self.model = Wav2Vec2Model.from_pretrained("facebook/wav2vec2-large-960h-lv60-self")
        embedding_dim = 512 ## adjust this
        encoder_dim = 256
        self.input_projection = nn.Sequential(
            nn.Linear(embedding_dim, encoder_dim),
            nn.Dropout(p=0.05),
        ...
        self.t_pooling = nn.AdaptiveMaxPool1d(50)
        self.sed_out_layer = nn.Sequential(
            nn.Linear(encoder_dim, encoder_dim),
            nn.LeakyReLU(),
            nn.Linear(encoder_dim, 13),
    ...
    def forward(self, x):
        # pdb.set_trace() # [32, 6, 251, 64]
        x=x.squeeze(1)
        conv_outputs = self.model.feature_extractor(x)
